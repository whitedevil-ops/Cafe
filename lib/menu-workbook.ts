...
export function downloadMenuTemplate(cafeName: string) {
  const rows: (string | number)[][] = [
    ['Category / Item', 'Price', 'Small', 'Medium', 'Large', 'Profit', 'Veg Type', 'Description'],
    ['BURGERS', '', '', '', '', '', '', ''],
    ['Classic Veg Burger', 149, '', '', '', 89, 'Veg', 'Classic vegetable burger'],
    ['Cheese Burger', 179, '', '', '', 104, 'Veg', 'Burger with cheese'],
    ['COLD DRINKS', '', '', '', '', '', '', ''],
    ['Cold Coffee', '', 80, 110, 130, '', 'Veg', 'Only fill sizes that apply — the rest can stay blank'],
    ['Coca Cola', 60, '', '', '', 35, 'Veg', ''],
  ]
  const ws = XLSX.utils.aoa_to_sheet(rows)
  ws['!cols'] = [{ wch: 26 }, { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 12 }, { wch: 34 }]
...
export type ExportRow = {
  ...
  /** Absolute prices for any Small/Medium/Large variant this item has —
      round-trips size options back through Export → edit → re-import. */
...
export function downloadMenuExport(cafeName: string, rows: ExportRow[]) {
  const header = ['Category', 'Item', 'Price', 'Small', 'Medium', 'Large', 'Profit', 'Veg Type', 'Description']
  ...
  ws['!cols'] = [{ wch: 20 }, { wch: 26 }, { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 9 }, { wch: 12 }, { wch: 34 }]
  const wb = XLSX.utils.book_new()
