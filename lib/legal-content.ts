// Central source for every legal document's content — app/legal/[doc]/page.tsx
// and components/legal/terms-modal.tsx both render from here, so the text a
// café owner scrolls through in the consent modal is always the exact same
// text the full /legal/terms page shows, never a second copy that can drift.
//
// The content below describes what KhaoPiyo ACTUALLY does — it was written
// from the data inventory in PRIVACY_COMPLIANCE_AUDIT.md (what is collected,
// where it is stored, which processors touch it).
//
...
export type Doc = { title: string; updated: string; intro: string; sections: Section[]; review?: string }
...
/**
 * The version identifier recorded alongside a consent acceptance. Reusing
 * each doc's own `updated` display date rather than a separate counter —
 * one fewer thing to remember to bump, and it already changes exactly when
 * the text a café owner is asked to accept changes.
 */
export function legalDocVersion(doc: keyof typeof LEGAL_DOCS): string {
  return LEGAL_DOCS[doc].updated
}
