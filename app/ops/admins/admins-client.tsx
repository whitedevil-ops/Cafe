...
export default function AdminsClient({
  ...
  async function toggleStatus(a: AdminRow) {
    ...
    const ok = await confirm({
      ...
      description: next === 'disabled' ? 'They will immediately lose access to the operator console.' : 'They will regain access to the operator console.',
  ...
  async function resetPassword(a: AdminRow) {
    ...
    const res = await fetch('/api/ops/admins/reset-password', {
...
function AddAdminDialog({ selfRole, onClose, onCreated }: { selfRole: string; onClose: () => void; onCreated: () => void }) {
  ...
  async function submit() {
    ...
    setSubmitting(true)
    const res = await fetch('/api/ops/admins/create', {
      method: 'POST',
  ...
  return (
    <DialogShell title="Add Admin" description="Creates a new operator-console login." onClose={onClose}>
