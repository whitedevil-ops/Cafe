...
export default async function TablesFloorPage() {
  ...
  // round-trip (items) it never actually depended on. live_tables joined the
  // same batch for the same reason — it was a blocking await before this
  // Promise.all, holding up every independent query below it.
  const [{ data }, { data: areas }, { data: categories }, { data: items }, smsBillsEnabled, waiterQuickAddEnabled, liveTablesEnabled] = await Promise.all([
    ...
    hasFeature(cafe.cafeId, 'live_tables'),
  ])

  if (!liveTablesEnabled) redirect('/dashboard')
