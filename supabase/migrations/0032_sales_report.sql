-- ============================================================================
-- 0032 — Sales reports: a date-range view of what actually happened,
-- distinct from the owner dashboard's "today so far" live counters.
--
-- WHY A SEPARATE DEFINITION OF "REVENUE" FROM THE DASHBOARD: dashboard.tsx's
-- today-count includes any non-cancelled order regardless of payment_status
-- (reasonable for "what's happening right now," where an order minutes old
-- is very likely to get paid same-session). A REPORT looking back over a
-- week or a month should reflect money that actually changed hands, so this
-- filters to payment_status = 'paid'. Two different, both-correct answers
-- to two different questions — not an inconsistency to "fix."
--
-- Existing indexes already cover this: orders(cafe_id, created_at desc) and
-- order_items(order_id) (schema.sql) — no new index needed.
-- ============================================================================
