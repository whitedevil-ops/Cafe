-- 0126 — claiming a spin prize on a POS bill.
--
-- 0125 shipped the wheel, the draw, and redeem_spin_prize, but left the till
-- with no honest way to actually apply a prize. Routing it through the staff
-- discount field looked obvious and is wrong twice over:
--
--   1. staff_place_order caps discounts by role (0123:958-968), converting
--      even a flat ₹ discount into a percent-equivalent and checking that.
--      A cashier capped at 5% cannot honour "Free Cold Coffee" on a ₹150
--      bill — but a won prize is the café's own promise, not that cashier's
--      discretion. It must not consume their discretionary budget either.
--   2. Redeeming from the browser before placing the order burns the prize
--      when the bill is abandoned, and leaves redeemed_order_id null, so the
--      owner can never see which bill a prize was spent on.
--
-- So the code travels with the order and is redeemed inside the same
-- transaction. redeem_spin_prize already takes its own advisory lock and
-- re-checks redeemed/expired, so two tills racing one code means the second
-- order raises and rolls back whole — no bill, no half-applied discount.
--
-- A free-item prize discounts a line that must already be on the bill, rather
-- than inserting one. The kitchen has to make the drink and inventory has to
-- deduct it, which only happens if it is a real line at a real price — the
-- same reasoning that made combo components real-priced rows in 0123.

-- Adding a parameter creates a second overload, and PostgREST cannot pick
-- between them. Drop the 13-argument signature before defining the 14th.
drop function if exists staff_place_order(
  uuid, jsonb, order_type, uuid, text, text, text, text, numeric, boolean, text, uuid, text);
...
begin
  ...
  -- Spin prize. Deliberately after the role cap and never measured against it:
  -- the café promised this on the wheel, so honouring it is not the cashier's
  -- discretion to spend. redeem_spin_prize does the locking and the
  -- redeemed/expired/'none' checks, and raises — rolling this order back
  -- entirely — if another till got there first.
  if p_spin_code is not null and trim(p_spin_code) <> '' then
    ...
    if v_spin.kind = 'item' then
      -- The won item has to be a real line: the kitchen makes it and stock
      -- deducts against it. Discount the dearest matching line so a guest who
      -- won "free Cappuccino" and ordered two gets the better one free.
...
revoke execute on function staff_place_order(
  uuid, jsonb, order_type, uuid, text, text, text, text, numeric, boolean, text, uuid, text, text) from public, anon;
grant execute on function staff_place_order(
  uuid, jsonb, order_type, uuid, text, text, text, text, numeric, boolean, text, uuid, text, text) to authenticated;
