-- ============================================================================
-- 0038 — Point both order engines at apply_order_taxes (0037).
--
-- WHY THE FUNCTIONS ARE REPLACED IN FULL rather than patched by a trigger:
-- both engines RETURN the total to their caller, and the POS shows that
-- number to the cashier before they take money. A deferred trigger that
-- corrected the stored total after the fact would leave the returned value
-- stale whenever per-item rates or inclusive pricing changed the answer —
-- i.e. the screen would disagree with the bill. For money, the value shown
-- and the value stored must come from the same computation.
--
-- Bodies are otherwise unchanged from 0025 (place_order) and 0016
-- (staff_place_order); only the tax tail differs. compute_bill() is left in
-- place and still used by the demo seed and smoke test.
-- ============================================================================
...
begin
  ...
  -- One canonical tax computation (0037). It writes subtotal/discount/tax/
  -- service_charge/total onto the order itself and hands the figures back.
