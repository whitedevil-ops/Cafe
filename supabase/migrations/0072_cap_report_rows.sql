-- ============================================================================
-- 0072 — gst_invoice_report and adjustments_report were the two report RPCs
-- (of 8) that returned every matching row with no LIMIT, unlike list_bills
-- (capped at 500) and payments_outstanding_report (capped at 100) — a café
-- querying a full year could get an unbounded JSON payload back in one call.
-- Summary totals/counts still run over the FULL unfiltered range (correctness
-- is unaffected); only the detail arrays are capped to the most recent 500
-- rows, same generous ceiling list_bills already uses.
-- ============================================================================
