-- ============================================================================
-- 0011 — Customer bill link + Café Profile foundations. Idempotent.
--  * place_order v5: returns receipt_token so the customer's done screen can
--    link their digital bill directly (SMS can't deliver until a provider is
--    configured, so this is currently the customer's only path to the bill).
--  * cafes: description/email/website columns for the profile page.
--  * cafe_settings.hours: per-day operating hours (jsonb).
--  * audit_logs: members may INSERT (0001 gave read-only; profile edits are
--    now audited). RLS unchanged otherwise.
-- ============================================================================
...
alter table cafe_settings add column if not exists hours jsonb not null default '{}';
