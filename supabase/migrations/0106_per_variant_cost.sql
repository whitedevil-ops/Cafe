-- ============================================================================
-- 0106 — Cost/profit can now differ per size (Small/Medium/Large), and it's
-- REAL — the actual size a customer ordered is what gets costed, so
-- Profitability Reports reflect true margin per size, not the base item's
-- cost applied uniformly regardless of which size sold.
--
-- Two additive pieces:
--   1. menu_item_variants.cost_delta — same shape as price_delta (0 by
--      default = "costs the same as the base item").
--   2. order_items.variant_id — which variant was actually ordered. Did not
--      exist before; the variant's NAME was folded into order_items.name
--      (e.g. "Cold Coffee (Large)") at sale time, but nothing structured
--      pointed back to menu_item_variants, so cost_snapshot had no way to
--      know which size's cost to freeze. (profitability_report already
--      groups by name, which already includes the size suffix — so once
--      cost_snapshot is size-aware, per-size margin needs no report changes.)
--
-- menu_item_effective_cost gains an optional p_variant_id — added via an
-- explicit DROP first, not a bare CREATE OR REPLACE, because this project
-- has hit the overload-ambiguity trap twice already (0043, 0097): replacing
-- a function while ADDING a parameter doesn't touch the old signature, it
-- creates a second overload alongside it, and PostgREST can't choose between
-- them on the next call. All existing 1-argument call sites keep working
-- unchanged (p_variant_id defaults to null → identical behaviour to before).
--
-- place_order and staff_place_order are otherwise byte-for-byte identical to
-- their current live bodies (0087 and 0078 respectively) — ONLY the
-- order_items insert gains a variant_id value, using v_variant_id, which
-- both functions already resolve and validate today. Copied verbatim from
-- source rather than retyped, specifically to avoid introducing any other
-- difference in the two functions every live Brewora order runs through.
-- ============================================================================

alter table menu_item_variants add column if not exists cost_delta integer not null default 0;
alter table order_items add column if not exists variant_id uuid references menu_item_variants(id) on delete set null;

-- ── menu_item_effective_cost: optional variant-aware overload ───────────────
drop function if exists menu_item_effective_cost(uuid);
...
-- ── Snapshot trigger: pass along which variant was actually sold ────────────
...
begin
  ...
  if new.cost_snapshot is null and new.menu_item_id is not null then
    new.cost_snapshot := menu_item_effective_cost(new.menu_item_id, new.variant_id);
  end if;
...
-- ── place_order (customer/QR-facing) — verbatim from 0087, order_items insert
--    gains variant_id only ─────────────────────────────────────────────────
...
-- ── staff_place_order (POS-facing) — verbatim from 0078, order_items insert
--    gains variant_id only ─────────────────────────────────────────────────
