-- ============================================================================
-- 0003 — Extend place_order to validate variants + add-ons server-side.
-- Same signature as 0002 (p_items jsonb) — each item object may now carry:
--   { "item_id":"uuid", "qty":2, "variant_id":"uuid"|null, "addon_ids":["uuid",...] }
-- All variant/add-on prices are looked up from the DB (never trusted from the
-- client), scoped to the same café and parent item. Unit price = base + variant
-- delta + add-on prices; the breakdown is snapshotted into order_items.modifiers.
-- ============================================================================
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    -- Variant (single, required if the item defines any)
